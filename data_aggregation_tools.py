...
def create_df_from_RIPE_RIS(ALL_RIS_PEERS):
    ...
    df.loc[list(unique_asns_v4), 'is_ris_peer_v4'] = 1
    df.loc[list(unique_asns_v6), 'is_ris_peer_v6'] = 1
    df.index.name = 'asn'
    asnList = list(df.index.values)
...
for ds in ALL_DATASETS_changes:
    ...
    # COMPARE_DF = COMPARE_DF._append(df, ignore_index=True)
