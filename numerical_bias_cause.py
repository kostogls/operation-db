...
import json
import random
from bias_utils import bias_score_dataframe, get_feature_type, get_features_dict_for_visualizations
from generate_distribution_plots import plot_all
from data_aggregation_tools import load_aggregated_dataframe
from itertools import pairwise


FEATS_KEYS = get_features_dict_for_visualizations()
...
# net_set_dict_names = ['RIPE Atlas (all)', 'RIPE RIS (all)']

# all_dict = {}
# platf_list_keys = ['RIPE Atlas', 'RIPE RIS', 'RouteViews', 'RIPE RIS & RouteViews', 'BGPtools']
...
for platf in net_set_dict_names:
    ...
    if platf == 'RIS':
        ...
        network_sets_dict[platf] = df.loc[
            (df['is_ris_peer_v4'] > 0) | (df['is_ris_peer_v6'] > 0) | (df['is_routeviews_peer'] > 0)]
    elif platf == 'bgptools':
    ...
    list_of_keys = ['AS_rank_numberAsns', 'AS_rank_numberPrefixes', 'AS_rank_numberAddresses', 'AS_rank_total',
                    'AS_rank_customer', 'AS_rank_peer', 'AS_rank_provider', 'peeringDB_ix_count', 'peeringDB_fac_count',
                    'peeringDB_info_prefixes4', 'peeringDB_info_prefixes6', 'nb_atlas_probes_v4', 'nb_atlas_probes_v6',
                    'AS_hegemony', 'cti_origin', 'cti_top']
    ...
    def combine_dictionaries(dict1, dict2):
        ...
        for key in dict1:
            if key in dict2:
                ...
                combined_values = {"-".join([str(j) for j in value2[i]]): value1[i] for i in
    ...
    # all_d.append(d)
    all_d[platf] = d
    all_d[platf] = rename_keys(all_d[platf], FEATS_KEYS)
    del all_d[platf]['peeringDB_info_prefixes4']
    del all_d[platf]['peeringDB_info_prefixes6']
    del all_d[platf]['nb_atlas_probes_v4']
    del all_d[platf]['nb_atlas_probes_v6']
