...
def get_bgptools_data(save_filename, dataset):
    headers = {
    ...
    if save_filename is not None:
        path = '../../data/misc/'
        # print(saved)
        df = pd.DataFrame(saved, columns=['unnamed', 'ASN', 'name', 'v4', 'v6'])
        df = df.iloc[1:, :]
        ...
        # if dataset == 'simple_list':
        #     all_asns.to_csv(save_filename, index=False)
        # elif dataset == 'v4':
        #     v4.to_csv(save_filename, index=False)
        # elif dataset == 'v6':
        #     v6.to_csv(save_filename, index=False)
        ...
        if dataset == 'all':
            df_asns.to_csv(save_filename, index=False)
        # df_asns.to_csv(save_filename, index=False)
    else:
        return saved
...
def AS_rank_collector(save_filename, window=500, offset=0):
    ...
    while has_next_page:
        ...
        print('\t step {} ({}) \t total time {} sec\r'.format(i, round(r['data']['asns']['totalCount'] / window),
                                                              round(time.time() - t)), end='')
...
def super_collector(save_folder):
    ...
    tjoin = lambda x: os.path.join(save_folder, x.format(DATE))

    d = {
        'RIPE RIS collectors': {'func': ripe_ris_collector, 'kwargs': {'save_filename': 'RIPE_RIS_collectors_{}.json'}},
        'RIPE Atlas probes': {'func': ripe_atlas_probes_collector,
                              'kwargs': {'save_filename': 'RIPE_Atlas_probes_{}.json'}},
        'IHR AS Hegemony': {'func': AS_hegemony_collector, 'kwargs': {'save_filename': 'AS_hegemony_{}.csv'}},
        'BGPtools personal AS': {'func': bgp_tools_collector, 'kwargs': {'save_filename': 'bgp_perso_{}.txt'}},
        ...
        'bgptools all': {'func': get_bgptools_data, 'kwargs': {'save_filename': 'bgptools_{}.csv', 'dataset': 'all'}}
...
super_collector('./data/misc')

ALL_DATASETS = ['AS_hegemony', 'AS_relationships', 'ASDB', 'ASrank', 'bgptools', 'bgptools_perso', 'CTI_origin', 'CTI_top',
...
# na petaei warning an new file < old file
# concept drift
# compare distributions in two agg dfs (later)

# vgale gia kathe dataset tin lista me ta asn poy periexei kai sygkrine ta me to neo dataset poy erxetai

# tha boune san collections sthn vash
