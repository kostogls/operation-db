...
#
# from app.project_files.data_aggregation_tools import load_aggregated_dataframe
#
#
...
#
#     bias_df, bias_per_bins, bins= bias_score_dataframe(df[FEATURES], {imp: df_mon[FEATURES]}, **params)
#
#     return bias_df, bias_per_bins, bins
#
#
...
# 								  'nb_atlas_probes_v6', 'AS_rel_degree', 'cti_top', 'cti_origin']
...
# 			bias_df.loc[col, s_name], bias_per_bin, bins = bias_score(ds1, ds2, **params)
# 	return bias_df, bias_per_bin, bins
#
#
...
# 		'cti_origin': 'Country influence (CTI origin)',
# 		'cti_top': 'Country influence (CTI top)',
# 	    # Topology info
...
def get_distribution_vectors_from_data(target_data, sample_data, data_type, nb_bins=None):
	...
	# print(q)
		...
		p = p.values
		q = pd.DataFrame(sample_data).value_counts()
		q = q.reindex(index=bins,
					  fill_value=0)  # use the rows of q in the order of rows in p; for non-existing indices in q, it sets 0
		q = q / len(sample_data)
		q = q.values
	else:
		raise ValueError
...
def get_bias_from_data(method, target_data, sample_data, data_type, **params):
	...
	:return:		(float, list, list)
						...
						- the bins (for 'numerical') or the categories (for 'categorical') corresponding to the list of Total Variation values
	'''
	p, q, bins = get_distribution_vectors_from_data(target_data, sample_data, data_type,
													nb_bins=params.get('bins', None))
	# print(bins)
	if method == 'kl_divergence':
		bias_score, bias_per_bin = kl_divergence(p, q, alpha=params['alpha'])
	elif method == 'total_variation':
		...
		abs_diff = [np.abs(p[i] - q[i]) for i in range(len(p))]
		bias_score = 0
		return abs_diff
	else:
		raise ValueError
	return bias_score, p, q, bins
...
def bias_score(target_data, sample_data, method='kl_divergence', **params):
	...
	'''
	if method in ['kl_divergence', 'total_variation', 'max_variation']:
		bias_score, p, q, bins = get_bias_from_data(method, target_data, sample_data, **params)
	# print(bins)
	elif method == 'ks_test':
		statistic, p_value = kstest(target_data, sample_data)
		bias_score = 1 - p_value
		...
		abs_diff = get_bias_from_data(method, target_data, sample_data, **params)
		return abs_diff
	else:
		raise TypeError
	return bias_score, p, q, bins
...
def bias_score_dataframe(target_df, sample_df_dict, preprocess=True, **params):
	bias_df = pd.DataFrame(index=target_df.columns)
	bins = pd.DataFrame(index=target_df.columns)
	...
	for col in target_df.columns:
		...
		for s_name, s_df in sample_df_dict.items():
			...
			if preprocess:
				ds2 = preprocess_data_series(ds2)
			params['data_type'] = get_feature_type(col)
			bias_df.loc[col, s_name], p, q, bins = bias_score(ds1, ds2, **params)
			# print(bins)
			plist.append(p)
			qlist.append(q)
			bins_list.append(bins)
	return bias_df, plist, qlist, bins_list
...
def subsampling_naive_sorting(df, set_of_monitors, bias_params, FEATURES, bias_weights=None):
	...
	bias_df, _, _, _ = bias_score_dataframe(df[FEATURES], network_sets_dict_for_bias, **bias_params)
	if bias_weights is None:
		...
		print(bias_df)
		# print(bias_diff)
		muldf = bias_df.mul(pd.Series(bias_weights), axis=0)
		bias_diff = muldf.sum(axis=0)
...
def get_features_dict_for_visualizations():
	...
	features_dict = {
		# Location-related info
		'AS_rank_source': 'RIR region',
		'AS_rank_iso': 'Location (country)',
		'AS_rank_continent': 'Location (continent)',
		# network size info
		'AS_rank_numberAsns': 'Customer cone (#ASNs)',
		'AS_rank_numberPrefixes': 'Customer cone (#prefixes)',
		'AS_rank_numberAddresses': 'Customer cone (#addresses)',
		'AS_hegemony': 'AS hegemony',
		'cti_origin': 'Country influence (CTI origin)',
		'cti_top': 'Country influence (CTI top)',
		# Topology info
		'AS_rank_total': '#neighbors (total)',
		'AS_rank_peer': '#neighbors (peers)',
		'AS_rank_customer': '#neighbors (customers)',
		...
		'ASDB_C1L1': 'ASDB C1L1',
		'ASDB_C1L2': 'ASDB C1L2',
		'peeringDB_info_type': 'Network type (PeeringDB)',
		'peeringDB_info_ratio': 'Traffic ratio (PeeringDB)',
		'peeringDB_info_traffic': 'Traffic volume (PeeringDB)',
		'peeringDB_info_scope': 'Scope (PeeringDB)',
		'is_personal_AS': 'Personal ASN',
...
def get_bias_of_monitor_set(df=None, imp='Custom set', monitor_list=None, params=None):
	...
	bias_df,_,_,_ = bias_score_dataframe(df[FEATURES], {imp: df_mon[FEATURES]}, **params)
